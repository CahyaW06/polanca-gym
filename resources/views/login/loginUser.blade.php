    ...
    <form class="max-w-xs mx-auto" href="/login" method="POST">
      @csrf
        ...
        <div class="mb-5">
          ...
          <input type="password" id="password" name="password" class="bg-gray-50 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5 placeholder-gray-700 text-gray-700 @error('password') is-invalid @enderror" required value="user"/>
        </div>
        @error('password')
        <div class="alert-danger mb-5 -mt-3 text-red-400 text-xs">
          {{ $message }}
